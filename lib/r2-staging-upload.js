...
/**
 * Under `gallery/` so R2 tokens scoped to `gallery/*` still work. Avoids
 * `staging/*` when the token only allows the gallery prefix.
 */
const STAGING_PREFIX = "gallery/_staging/";
/** Per-chunk cap under Vercel's ~4.5 MB request limit (3 MiB for margin). */
const STAGING_CHUNK_BYTES = 3 * 1024 * 1024;
