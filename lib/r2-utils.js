...
function formatR2Error(err) {
  ...
  if (
    ...
    return (
      msg +
      " Check R2 API token permissions: Object Read & Write on this bucket (PutObject, GetObject, DeleteObject). Do not use a prefix-only token unless it includes gallery/*."
    );
  }
  return msg;
}
